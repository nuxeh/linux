...
static int __device_reset(struct device *dev)
{
	static void __iomem *reg;

	if (!reg)
		reg = ioremap(0xC0004414, 4);
	if (!reg)
		return -EINVAL;

	writel(readl(reg) | (1 << 10), reg);
	msleep(10);
	writel(readl(reg) & ~(1 << 10), reg);
	msleep(10);
...
static int meson_rtc_probe(struct platform_device *pdev)
{
	...
	if (1) {
		void __iomem *reg = ioremap(0xC0004414, 4);
		if (!reg)
			return -EINVAL;
